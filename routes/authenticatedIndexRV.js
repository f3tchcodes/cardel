...
router.get("/dashboard", async (req, res) => {
    try {
        ...
        console.log(err);
        return res.json({
            error: "JWT can't be verified, login again or contact us to fix.",
        });
    }
...
router.get("/onboarding", async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            error: "Server could not send the query to the database, if this persists please contact us at our support email.",
        });
    }
        ...
        if (rows1[0].first_time_login === 1) {
            try {
                ...
                const [rows2] = await con.query(
                    "SELECT subName, subIconURL FROM subscription_templates",
                );

                subscriptions = rows2.map((row) => [
                    row.subName,
                    row.subIconURL,
                ]);
                ...
                console.log(err);
                return res.json({
                    error: "Can't onboard you, contact us at our email to fix this error.",
                });
            }
        }
    } else {
