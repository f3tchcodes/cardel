...
router.post("/complete", async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            error: true,
            message:
                "Server could not send the query to the database, if this persists please contact us at our support email.",
        });
    }
    ...
    if (rows1.length === 1) {
        if (rows1[0].first_time_login === 1) {
            ...
            if (
                ...
                return res.json({
                    error: true,
                    mesage: "Input data is too large fucker, you tryna DoS or smth?",
                });
            }
            ...
            try {
                ...
                if (err.code == "ER_DUP_ENTRY") {
                    ...
                    return res.status(500).json({
                        error: true,
                        message:
                            "Onboarding is already done, refresh the page to fix this error. If you can't get out of this loop, contact us at our support email!",
                    });
                }
                return res.status(500).json({
                    error: true,
