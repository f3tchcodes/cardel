...
router.post("/signup", async (req, res) => {
    try {
        ...
        // basic checks
        if (username.length === 0 || normalizedEmail.length === 0 || password.length === 0){
            return res.json({ 
                success: false, 
                error: true, 
        ...
        await con.query(`INSERT INTO users 
            ...
            [username, normalizedEmail, hashed_password, marketing_consent, verificationToken, verificationExpires, nextVerificationAllowed]);
            ...
            // check if it has been 10 minutes since last time requesting email
                if (rows[0].next_verification_allowed < new Date(Date.now())){
                    ...
                    await con.query(`
                        ...
                        [verificationToken, verificationExpires, nextVerificationAllowed]
                    );
                    ...
                    return res.json({ 
                        success: false, 
                        error: true, 
                        message: "You can only send one verification email every 5 minutes.",
                    });
                }
            } catch(err){
                console.log(err);
