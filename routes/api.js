...
router.get("/v1/user", jwtAuth, (req, res) => {
    try{
...
// complete onboarding
router.post("/v1/complete-onboarding", async (req, res) => {
...
router.get("/v1/user/subscriptions/list", async (req, res) => {
    try{
        jwt_data = jwt.verify(req.cookies.token, process.env.JWT_SECRET);
    } catch(err){
        console.log(err);
...
router.post("/v1/user/subscriptions/toggle", async (req, res) => {
    try{
        jwt_data = jwt.verify(req.cookies.token, process.env.JWT_SECRET);
    } catch(err){
        console.log(err);
...
router.post("/v1/user/subscriptions/add", userSubIconsUpload.single('sub_icon'), async (req, res) => {
