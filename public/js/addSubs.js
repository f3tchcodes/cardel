...
const plusBtn =
    document.querySelector(".plus-btn") || document.querySelector(".btn-plus");
...
if (plusBtn) {
    ...
    console.warn(
        "Plus button not found! Make sure your button has the class 'plus-btn'",
    );
}
...
document.querySelectorAll("#cdl-drop-unit .dropdown-item").forEach((item) => {
    item.addEventListener("click", () => {
        ...
        document.querySelector("#cdl-drop-unit .selected").textContent =
            item.textContent;
...
// --- ICON PREVIEW ---
document
    .getElementById("cdl-iconInput")
    .addEventListener("change", function () {
        if (this.files && this.files[0]) {
            const reader = new FileReader();
            reader.onload = (e) =>
                (document.getElementById("cdl-iconPreview").src =
                    e.target.result);
