...
const updateEmailTemplate = path.resolve(__dirname, '../views/emails/subUpdateEmail.ejs');
const renewalEmailTemplate = path.resolve(__dirname, '../views/emails/subRenewalEmail.ejs');
...
async function subEmail(status, job_id, days_left = 3){
    try {
        ...
        if (status === "update") {
            ...
            await ejs.renderFile(updateEmailTemplate, emailData, async (err, html) => {
        ...
        if (status === "renewal") {
            ...
            await ejs.renderFile(renewalEmailTemplate, emailData, async (err, html) => {
                if (err) return console.error('Error rendering email template:', err);
                await resend.emails.send({
                    from: `Cardel <${process.env.EMAIL}>`,
                    to: user.email,
